...
struct Particle
{
   vec4 pos;  //.xyz = pos,     .w = enabled/disabled
...
const int WIDTH = 128; //width and height of initial particle grid
const int HEIGHT = 32;
...
vec4 boundary_sdf(vec2 p)
{
    ...
    const vec3 plane0 = vec3(0.0, 1.0, -0.5*VIEW_HEIGHT+15.0*PARTICLE_RADIUS-PARTICLE_RADIUS+bottom);
    const vec3 plane1 = vec3(1.0, 0.0, -PARTICLE_RADIUS);
    const vec3 plane2 = vec3(-1.0, 0.0, VIEW_WIDTH-PARTICLE_RADIUS);
    const vec3 plane3 = vec3(0.0, -1.0, VIEW_HEIGHT-PARTICLE_RADIUS);
    ...
    float d3 = sdPlane(p, plane3);
    float d4 = sdCircle(p-cen, r);

    vec4 res = vec4(plane0.xy, d0, 0.0);
    res = opU(res, vec4(plane1.xy, d1, 1.0));
    res = opU(res, vec4(plane2.xy, d2, 2.0));
    res = opU(res, vec4(plane3.xy, d3, 3.0));
    //res = opU(res, vec4(normalize(p-cen), d4, 4.0));
    return res;
}
...
{
    float coord = x/VIEW_WIDTH;
    vec4 w = texture(wave_tex, coord);
    float h = w[0];
    return h;
}
vec4 GetWaveNormalHeight(float x, out float xvel)
{
    float coord = x/VIEW_WIDTH;
    float size = textureSize(wave_tex, 0);

    vec4 w = texture(wave_tex, coord);
    vec4 we = texture(wave_tex, coord-1.0/size);
    vec4 ww = texture(wave_tex, coord+1.0/size);

    float h = w[0];
    float he = we[0];
    float hw = ww[0];

    vec2 n = normalize(vec2(he-hw, 1.0));
    xvel = w[1]/w[0];
    return vec4(n, 0.0, h);
}
...
void InitParticle(int ix)
{
    ...
    vec4 p = init_grid(ix);
    p.x += PARTICLE_RADIUS;
    p.y += 0.5*VIEW_HEIGHT - 15.0*PARTICLE_RADIUS + PARTICLE_RADIUS;
...
vec4 init_grid(int ix)
{
    ...
    vec2 xy = vec2(ij)/vec2(cols, rows);
	vec2 p = vec2(VIEW_WIDTH, 18.0*H)*vec2(xy);
...
void ComputeDensityPressure(int ix)
{
    ...
    //enable/disable
    float xvel;
    vec4 w = GetWaveNormalHeight(pi.pos.x, xvel);

    float xv_thresh = 0.05;
    //enable particles within moving wave region
    if(pi.pos.w == 0.0 && abs(xvel)>xv_thresh) 
    {
        pi.pos.w = 1.0; //enable
    }

    //disable particles below wave where velocity is 0
    if(pi.pos.w == 1.0 && abs(xvel)<xv_thresh) 
    {
        pi.pos.w = 0.0; //disable
    }
    ...
    //add density contribution from wave
    //float coord = pi.pos.x/VIEW_WIDTH;
    //vec4 w = texture(wave_tex, coord);
    //float h = 10.0*w.x+1.0;
    //float r = abs(pi.pos.y-h);
    //rho += W_cubic(r);
...
void ComputeForces(int ix)
{
    ...
    if(db.z<H)
    {
        ...
        float Wgrad = W_cubic_grad(max(0.0, r+0.0*PARTICLE_RADIUS));
        //add boundary component to pressure force
        acc_press += PSI*acc_press_i*Wgrad*uij;
    }

    //add pressure contribution from wave
    float vx;
    vec4 wave = GetWaveNormalHeight(pi.pos.x, vx);
    float h = wave.w;
    vec2 vw = vec2(vx, 0.0);
    {
        float r = max(0.0, pi.pos.y-h);
        if(r<H)
        {
            float Wgrad = W_cubic_grad(max(0.0, r));
            vec2 uij = -wave.xy;
            //add boundary component to pressure force
            //acc_press += PSI*acc_press_i*Wgrad*uij;

            vec2 vij = pi.vel.xy - vw;
            vec2 rij = r*uij;
            float rho_j = rho_i;

            //acc_visc -= c_visc*(1.0/rho_j * dot(vij, rij)/(r*r + 0.01*HSQ) * Wgrad*uij);
        }
    }

    float dh = h-VIEW_HEIGHT/2.0;
    float wave_mask = smoothstep(0.0, 0.2, abs(vx));
    float tro_phase = 0.0;//0.5*pi.pos.x;
    float tro_speed = -0.5*sign(vx);
    float tro_mag = 0.0*abs(vx)*wave_mask;
    float tro_ang = tro_speed*time+tro_phase;
    vec2 acc_tro = vec2(0.0);
    //acc_tro += tro_mag*vec2(sin(tro_ang), -cos(tro_ang));// + vw + wave.xy;
    //acc_tro += 0.1*vec2(wave.y, -wave.x)*vx;
    //float dy = pi.pos.y-h;

    
    //float dy = pi.pos.y-1.1*h;
    float dy = pi.pos.y-h;
    dy -= 0.5*dh;

    if(ix%5<4)
    acc_tro.y -= wave_mask*500000.0*smoothstep(0.0, 10.0, dy); //wave pressure (vertical)
    //acc_tro.xy -= 0.1*wave.xy*wave_mask*500000.0*smoothstep(0.0, 2.0, dy); //wave pressure (normal)

    //INTEGRATE
    //forward Euler integration
    pi.acc.xy = acc_press + acc_visc + acc_grav + acc_tro;
    ...
    if(pi.pos.y < h)
    {
        //float depth = h-pi.pos.y;
        //pi.pos.y += 0.50*depth;
        

        //pi.vel.xy += 2.0*wave.xy*vec2(10.0, 1.0);
        //pi.vel.xy += 2.0*vw;
    }
