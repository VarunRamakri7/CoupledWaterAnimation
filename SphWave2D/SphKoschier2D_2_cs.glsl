...
const int HEIGHT = 32;
const float VIEW_WIDTH = 3*WIDTH*PARTICLE_DIAM;
const float VIEW_HEIGHT = 3*HEIGHT*PARTICLE_DIAM;
...
vec4 boundary_sdf(vec2 p)
{
    ...
    const vec2 cen = vec2(0.5*VIEW_WIDTH, 0.5*VIEW_HEIGHT);
    const float r = 0.25;
...
void InitParticle(int ix)
{
    ...
    p.xy += 1.2/6.0*vec2(VIEW_WIDTH, VIEW_HEIGHT);
    p.xy += vec2(1.0);
...
vec4 init_grid(int ix)
{
	...
	vec2 p = PARTICLE_DIAM*(ij-ivec2(rows/2, cols/2));
...
void ComputeDensityPressure(int ix)
{
	...
	*/
    ...
    for(int jx=0; jx<num_particles; jx++)
    {
        //include particle ix in density calculations
    ...
    if(db.z<H)
    {
        rho += PSI*W_cubic(max(0.0, db.z+PARTICLE_RADIUS));
        //rho += gamma(db.z)*W_cubic(max(0.0, db.z));
        //rho += PSI*W_cubic(max(0.0, db.z))*Vb(db.z);
    }

    rho = max(REST_DENS, rho); //clamp density to avoid negative pressures
    set_rho(pi, rho);
...
void ComputeForces(int ix)
{
    ...
    if(db.z<H)
    {
        float Wgrad = W_cubic_grad(max(0.0, db.z+PARTICLE_RADIUS));
        vec2 uij = -db.xy;
        //add boundary component to pressure force
        //acc_press -= PSI*Wgrad*uij;
        acc_press += PSI*acc_press_i*Wgrad*uij;
        //acc_press -= Vb(db.z)*acc_press_i*PSI*Wgrad*uij;
        //acc_press -= Vb(db.z)*PSI*Wgrad*uij;
        //acc_press -= Vb(db.z)*PSI*(acc_press_i+(get_pressure(pi))/(PSI*PSI))*Wgrad*uij; //correct
        //acc_press -= Vb(db.z)*PSI*Wgrad*uij;
  
        //add viscosity contribution from boundary
        //acc_visc += acc_press_i*Wgrad*uij;

        //float r = db.z;
        //vec2 rij = db.z*uij;
        //vec2 tv = pi.vel.xy - uij*dot(uij, pi.vel.xy);
        //acc_visc -= 1.0/PSI * dot(tv, rij)/(r*r + 0.01*HSQ) * Wgrad*uij;
    }
