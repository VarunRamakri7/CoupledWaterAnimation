...
const float H = 8.0;//16.0;           // kernel radius
...
const float SPIKY_GRAD = -10.f / (M_PI * pow(H, 5.0));
const float VISC_LAP = 40.f / (M_PI * pow(H, 5.0));
...
void ComputeDensityPressure(int ix)
{
	...
	//pi.force.w = rho;
    //pi.vel.w = GAS_CONST * (rho - REST_DENS); 
    set_rho(pi, rho);
